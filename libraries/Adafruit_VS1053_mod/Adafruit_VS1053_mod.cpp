...
SdFile Adafruit_VS1053::track;
SdFat Adafruit_VS1053::SD_CARD;
...
void Adafruit_VS1053::refill() {

  while(digitalRead(_dreq)) {

    if(!track.read(mp3buffer, sizeof(mp3buffer))) { //Go out to SD card and try reading 32 new bytes of the song
...
void Adafruit_VS1053::cs_low() {
  spiInit();
  digitalWrite(_cs, LOW);
}

void Adafruit_VS1053::cs_high() {
  digitalWrite(_cs, HIGH);
}

void Adafruit_VS1053::dcs_low() {
  spiInit();
  digitalWrite(_dcs, LOW);
}

void Adafruit_VS1053::dcs_high() {
  digitalWrite(_dcs, HIGH);
}


uint16_t Adafruit_VS1053::Mp3ReadWRAM (uint16_t addressbyte){
...
void Adafruit_VS1053::Mp3WriteWRAM(uint16_t addressbyte, uint16_t data){
...
void Adafruit_VS1053::flush_cancel(flush_m mode) {
  ...
  if((mode == post) || (mode == both)) {
    ...
    for(int y = 0 ; y < 2052 ; y++) {
      while(!digitalRead(_dreq)); // wait until DREQ is or goes high
  ...
  for (int n = 0; n < 64 ; n++)
  {
//    sciWrite(SCI_MODE, SM_LINE1 | SM_SDINEW | SM_CANCEL); // old way of SCI_MODE WRITE.
...
//  sciWrite(SCI_MODE, SM_LINE1 | SM_SDINEW | SM_RESET); // old way of SCI_MODE WRITE.
...
//  vs_init(); // perform hardware reset followed by re-initializing.
  //vs_init(); // however, Adafruit_VS1053_FilePlayer::begin() is member function that does not exist statically.
}


uint8_t Adafruit_VS1053_FilePlayer::skip(int32_t timecode){
  ...
  if(playingMusic) {
    ...
    //sciWrite(VS1053_REG_VOLUME, 0xFEFE);
 
    //sciWrite(VS1053_REG_VOLUME, 0xFEEF);
    sciWrite(VS1053_REG_DECODETIME, timecode);
    //seeked successfully
...
boolean Adafruit_VS1053_FilePlayer::startPlayingFile(const char *trackname) {
  ...
  if(!track.open(trackname, O_READ)) return false;

  // find length of arrary at pointer
  int fileName_length = 0;
  while(*(trackname + fileName_length))
    fileName_length++;
...
uint16_t Adafruit_VS1053::loadPlugin(char *plugname) {
  ...
  File plugin = SD_CARD.open(plugname);
  if (!plugin) {
...
uint16_t Adafruit_VS1053::getVolume() {
 
  uint16_t MP3SCI_VOL = sciRead(VS1053_REG_VOLUME);
  return MP3SCI_VOL;
}
...
#if (1)
  	SD_CARD.begin(_cardCS);
	if (int8_t(SD_CARD.vol()->fatType()) == 0) {
